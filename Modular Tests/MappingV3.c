...
float Turn(float angle, struct Robot car);
  ...
  while(angle > .1 && angle < 179.9) {   //in between 0 and 180 for IR
    ...
    if (pos < (ipos+360)) {           //Turn IR clockwise
      ...
      while (diff < 720 && pos < ipos+720) {          //Less than half a turn
        ...
        angle = (diff)*360/1440;
        if(sens < 60 && sens > .75) {
          y = (int)(currY +(cos(((angle+heading-90)/180)*3.1415)*sens))/GridSize;
          x = (int)(currX + (sin(((angle+heading-90)/180)*3.1415)*sens))/GridSize;
...
float Turn(float angle, struct Robot car){
  ...
  int timesRollover, timesRolloverDes, close, rolls;
  unsigned long int T0;
  float driveR, driveL, rolloverDist, dist1, dist, heading;
