...
struct Point
  ...
  while(solved != 1){
    characterize(ipos, grid, currX, currY, heading);
    ...
    if(next.x != gridX && next.y != gridY){
      dist = sqrt(2*(SquareSize*SquareSize));
      //angle = atan((next.x-gridX)/(next.y-gridY))*180/3.1415;
      //if(angle<0){
      //  angle = angle+360;
      //}
      if(next.y>gridY && next.x>gridX){
        angle = 45;
      }
      else if(next.y<gridY && next.x>gridX){
        angle = 135;
      }
      else if (next.y<gridY && next.x<gridX) {
        angle = 225;
      ...
      else if(next.y <gridY){
        angle = 180;
      }
      else if (next.x>gridX) {
        angle = 90;
      }
      else {
        angle = 270;
     ...
     printf("Going to (%d,%d), angle:%f, heading %f \n", next.x, next.y, angle, car.heading);
    angle = angle - car.heading;
    if (angle < 0){
      angle = angle+360;
    }
    ...
    for(i=queue[curr].x-1; i < queue[curr].x+2; i++){
      ...
      //printf("looking at point (%d,%d)\n",i,j);
        if (i>=0 && j>=0 && j<GridSize && i<GridSize){
        	...
        	if(grid[i][j] < (Filter+1) && visited[i][j] == 0){
	          queue[end].x = i;
	          queue[end].y = j;
...
float Turn(float angle, float *heading, float *x, float *y){
  ...
  while(Rdesired-Rwheel > 55) {
   ...
   //needs help!!!!!!!!!!!!!!!!!!!!!!!!!
   //*x = *x + sin(angle+heading)*WheelRadius;
   //*y = *y + cos(angle+heading)*WheelRadius - WheelRadius;
